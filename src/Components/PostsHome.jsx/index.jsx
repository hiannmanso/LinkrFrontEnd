...
export default function PostHome(props) {
    const { name, userID, idLocal, openUrl, toEdit, Input, description, index, picture, like, postContent, url, likes, urlDescription, urlTitle, urlImage, id, quantityLikes, modalScreen, editTextOfPost } = props;
    ...
    async function send() {
        try {
            await api.editPost(idLocal, { description: editedTextRef.current }, token);
            setDisabled(false);
            setEditing(false);
        } catch (err) {
    ...
    return (
        <s.Post key={index}>
            <div className='icons'>
                ...
                {userID === idLocal ? <s.editPost><ion-icon onClick={() => {
                    setEditing(true);
                }} name="pencil"></ion-icon></s.editPost> : <></>}
                <Likes like={() => like(id)} likes={quantityLikes} id={id} />
            </div>
            <div className='description'>
                <div className='first-line'>
                    <p>{name}</p>
                    {id === idLocal ? <img src={trasher} alt="trasher" onClick={() => { modalScreen() }} /> : <></>}

                </div>
                {!editing ? <Edit description={editedDescription} /> :
                    <form onSubmit={() => toEdit(id)}>
                        <i.Input defaultValue={editedDescription}
                            ref={inputRef}
                            onFocus={(e) =>
